...
UnitTest.asynctest('Editor (Silver) Configuration Close test', (success, failure) => {
  ...
  TinyLoader.setup(
    (editor: Editor, onSuccess, onFailure) => {
      ...
      const cAssertToolbarButtonDisabled = (label, f: (s, str, arr) => StructAssert[]) => {
      ...
      Pipeline.async({ }, [
        Chain.asStep({}, Log.chains('TBA', 'Test if the toolbar buttons are disabled in readonly mode when toolbar drawer is present', [
          cAssertToolbarButtonDisabled('Assert the first toolbar button, Bold is disabled', (s, str, arr) => {
            return [ buttonStruct(s, str, arr, 'Bold'), s.theRest() ];
          }),
